@section('title', 'Layout | Edit Plc user')
@extends('dashboard')

@section('component')
<form class="w-full max-w-lg flex-col self-center" method="POST" action="/editPlcUser/{{$user[0]->id}}">
    @csrf
    ...
    <x-formInput>
          ...
          <input value="{{$user[0]->email}}" name="email" class="appearance-none block w-full  text-gray-700 border border-gray-300 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="email" type="text">
    </x-formInput>
    <x-formInput class="none">
        ...
        <select name="role" required
         ...
         <option selected hidden value="{{$user[0]->role}}">{{$user[0]->role}}</option>
