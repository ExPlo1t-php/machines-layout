@section('title', 'Layout | Manage plc users')
@extends('dashboard')
...
<form class="w-full max-w-lg flex-col self-center" method="POST" action="/addPlcUser">
    @csrf
  <x-formInput>
        <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="email">
          User email
        </label>
        <input name="email" class="appearance-none block w-full  text-gray-700 border border-gray-300 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="email" type="text">
  </x-formInput>
  <x-formInput class="none">
    ...
    <select name="role" required
     class="appearance-none block w-full  text-gray-700 border border-gray-300 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="role">
...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
  <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 ">
    <caption class="text-red-500">ADMIN:All priveleges;<br> USER2:Some priveleges;<br> USER1:Only viewing</caption>
      <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
          <tr>
            <th scope="col" class="px-6 py-3 w-1/6 cursor-pointer">
                User Id
            </th>
        ...
        @if(count($users) > 0)
          ...
          <tr class="border-b dark:bg-gray-800 dark:border-gray-700 odd:bg-white even:bg-gray-50 odd:dark:bg-gray-800 even:dark:bg-gray-700">
            ...
            <td class="px-4 py-4 text-right flex">
                <a data-id="{{$user['id']}}" data-method="get" href="{{route('plcUser', $user['id'])}}" id="edit" class="m-2 font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                <a data-id="{{$user['id']}}" data-method="DELETE" href="{{route('deletePlcUser', $user['id'])}}" id="delete" class="m-2 font-medium text-red-600 dark:text-red-500 hover:underline">Delete</a>
            </td>
          </tr>
          @endforeach
        @else
